...
Ext.define("Ext.util.Observable", {
			...
			for (d in c) {
				if (c.hasOwnProperty(d)) {
					...
					if (!g.eventOptionsRe.test(d)) {
						g.addManagedListener(h, d, b.fn || b, b.scope
										|| c.scope, b.fn ? b : c)
					}
				}
			}
			...
			for (c in k) {
				if (k.hasOwnProperty(c)) {
					...
					if (!e.eventOptionsRe.test(c)) {
						e.removeManagedListener(h, c, b.fn || b, b.scope
										|| k.scope)
					}
				}
			}
	...
	fireEvent : function(b) {
		var c = b.toLowerCase(), d = this.events, e = d && d[c], a = e
				&& e.bubble;
			...
			for (c in b) {
				if (b.hasOwnProperty(c)) {
					...
					if (!g.eventOptionsRe.test(c)) {
						g.addListener(c, a.fn || a, a.scope || b.scope, a.fn
										? a
										: b)
					}
				}
	...
	removeManagedListenerItem : function(b, a, f, c, e, d) {
		if (b
				|| (a.item === f && a.ename === c && (!e || a.fn === e) && (!d || a.scope === d))) {
	...
	createRelayer : function(a) {
		var b = this;
		return function() {
			return b.fireEvent.apply(b, [a].concat(Array.prototype.slice.call(
							arguments, 0, -1)))
		}
	},
...
Ext.define("Ext.util.ClickRepeater", {
			...
			handleMouseDown : function(a) {
				...
				this.timer = Ext.defer(this.click, this.delay || this.interval,
						this, [a])
			},
			click : function(a) {
				this.fireEvent("click", this, a);
				this.timer = Ext.defer(this.click,
						this.accelerate ? this.easeOutExpo(Ext.Date
										.getElapsed(this.mousedownTime), 400,
								-390, 12000) : this.interval, this, [a])
			},
...
Ext.define("Ext.util.TextMetrics", {
			...
			bind : function(a) {
				var b = this;
				b.el = Ext.get(a);
				b.measure.setStyle(b.el.getStyles("font-size", "font-style",
			...
			Ext.Element.addMethods({
						getTextWidth : function(c, b, a) {
							return Ext.Number.constrain(
									Ext.util.TextMetrics.measure(this.dom, Ext
													.value(c,
															this.dom.innerHTML,
															true)).width, b
											|| 0, a || 1000000)
...
Ext.define("Ext.util.Floating", {
	...
	constructor : function(a) {
		var b = this;
		b.floating = true;
		b.el = Ext.create("Ext.Layer", Ext.apply({}, a, {
							hideMode : b.hideMode,
							hidden : b.hidden,
							shadow : Ext.isDefined(b.shadow)
									? b.shadow
									: "sides",
	...
	getConstrainVector : function(b) {
		...
		if (c.constrain || c.constrainHeader) {
			...
			b = b || (c.floatParent && c.floatParent.getTargetEl())
					|| c.container;
	...
	setActive : function(b, c) {
		...
		if (b) {
			...
			if ((a instanceof Ext.window.Window)
					&& (c instanceof Ext.window.Window)) {
				a.el.disableShadow()
			}
...
Ext.define("Ext.dd.DragTracker", {
	...
	constructor : function(a) {
		...
		this.addEvents("mouseover", "mouseout", "mousedown", "mouseup",
				"mousemove", "beforedragstart", "dragstart", "dragend", "drag");
	...
	onMouseDown : function(b, a) {
		...
		if (this.fireEvent("mousedown", this, b) === false
				|| this.fireEvent("beforedragstart", this, b) === false
	...
	constrainModes : {
		...
		dragTarget : function(c, f) {
			var b = c.startXY, e = c.startRegion.copy(), a = c
...
Ext.define("Ext.layout.component.Component", {
	...
	beforeLayout : function(b, j, k, h) {
		...
		var g = this, c = g.owner, d = c.ownerCt, f = c.layout, e = c
				.isVisible(true), a = c.el.child, i;
		g.previousComponentSize = g.lastComponentSize;
		if (!k
				&& ((!Ext.isNumber(b) && c.isFixedWidth()) || (!Ext.isNumber(j) && c
						.isFixedHeight())) && h && h !== d) {
			g.doContainerLayout();
			return false
		}
	...
	doOwnerCtLayouts : function() {
		var b = this.owner, e = b.ownerCt, c, h, d = this.lastComponentSize, g = this.previousComponentSize, a = (g
				&& d && Ext.isNumber(d.width)) ? d.width !== g.width : true, f = (g
				&& d && Ext.isNumber(d.height)) ? d.height !== g.height : true;
		if (!e || (!a && !f)) {
			return
		}
		c = e.componentLayout;
...
Ext.define("Ext.chart.Shape", {
			...
			triangle : function(a, b) {
				...
				return a.add(Ext.apply({
							type : "path",
							stroke : null,
							path : "M".concat(b.x, ",", b.y, "m0-", b.radius
											* 0.58, "l", b.radius * 0.5, ",",
									b.radius * 0.87, "-", b.radius, ",0z")
						}, b))
			},
			...
			plus : function(a, c) {
				...
				return a.add(Ext.apply({
							type : "path",
							stroke : null,
							path : "M".concat(c.x - b / 2, ",", c.y - b / 2,
									"l", [0, -b, b, 0, 0, b, b, 0, 0, b, -b, 0,
											0, b, -b, 0, 0, -b, -b, 0, 0, -b,
											"z"])
						}, c))
			},
			arrow : function(a, c) {
				...
				return a.add(Ext.apply({
							type : "path",
							path : "M".concat(c.x - b * 0.7, ",",
									c.y - b * 0.4, "l", [b * 0.6, 0, 0,
											-b * 0.4, b, b * 0.8, -b, b * 0.8,
											0, -b * 0.4, -b * 0.6, 0], "z")
						}, c))
			},
			drop : function(b, a, f, e, c, d) {
				...
				b.add({
							type : "path",
							path : ["M", a, f, "l", c, 0, "A", c * 0.4,
									c * 0.4, 0, 1, 0, a + c * 0.7, f - c * 0.7,
									"z"],
							fill : "#000",
							stroke : "none",
							rotate : {
...
Ext.define("Ext.ZIndexManager", {
			...
			_showModalMask : function(a) {
				var d = a.el.getStyle("zIndex") - 4, b = a.floatParent
						? a.floatParent.getTargetEl()
						: Ext.get(a.getEl().dom.parentNode), c;
				if (!b) {
					return
				}
				c = b.getBox();
			...
			_hideModalMask : function() {
				if (this.mask && this.mask.dom.parentNode) {
					Ext.get(this.mask.dom.parentNode)
							.removeCls(Ext.baseCSSPrefix + "body-masked");
					this.mask.hide()
				}
			},
			...
			_onContainerResize : function() {
				if (this.mask && this.mask.isVisible()) {
					this.mask.setSize(Ext.get(this.mask.dom.parentNode)
							.getViewSize(true))
				}
			},
			register : function(a) {
...
Ext.define("Ext.layout.component.AbstractDock", {
	...
	adjustAutoBox : function(h, l) {
		var b = this.info, a = this.owner, m = b.bodyBox, q = b.size, j = b.boxes, f = j.length, o = h.type, e = this.frameSize, p = b.padding, d = b.border, c = b.autoSizedCtLayout, n = (f < l)
				? f
				: l, g, k;
		if (o == "top" || o == "bottom") {
			for (g = 0; g < n; g++) {
	...
	initBox : function(j) {
		var h = this, g = h.info.bodyBox, a = (j.dock == "top" || j.dock == "bottom"), b = h.owner, e = h.frameSize, c = h.info, i = c.padding, d = c.border, f = {
			...
			overlay : j.overlay,
			type : j.dock,
			offsets : Ext.Element.parseBox(j.offsets || {}),
...
Ext.define("Ext.chart.Mask", {
	...
	resized : function(d) {
		var f = this, k = f.bbox || f.chartBBox, i = k.x, h = k.y, a = k.width, l = k.height, c = f.mask
				.getBox(true), g = Math.max, b = Math.min, m = c.x - i, j = c.y
				- h;
		m = g(m, i);
		j = g(j, h);
		m = b(m, a);
	...
	onMouseMove : function(s) {
		var t = this, n = t.maskType, a = t.bbox || t.chartBBox, h = a.x, g = a.y, b = Math, p = b.floor, i = b.abs, m = b.min, o = b.max, j = p(g
				+ a.height), l = p(h + a.width), d = s.getPageX(), c = s
				.getPageY(), r = d - t.el.getX(), q = c - t.el.getY(), f = t.maskMouseDown, k;
...
Ext.define("Ext.chart.Navigation", {
	...
	setZoom : function(a) {
		var c = this, f = c.axes, g = c.chartBBox, e = 1 / g.width, b = 1
				/ g.height, d = {
			x : a.x * e,
			y : a.y * b,
			width : a.width * e,
		...
		f.each(function(i) {
					...
					if (i.position == "bottom" || i.position == "top") {
						var k = (h.to - h.from) * d.x + h.from, j = (h.to - h.from)
								* d.width + k;
						i.minimum = k;
						i.maximum = j
					} else {
						var j = (h.to - h.from) * (1 - d.y) + h.from, k = j
								- (h.to - h.from) * d.height;
						i.minimum = k;
						i.maximum = j
					}
				});
...
Ext.define("Ext.data.Connection", {
	...
	request : function(j) {
		...
		if (f.fireEvent("beforerequest", f, j) !== false) {
			...
			if ((j.cors === true || f.cors === true) && Ext.isIe
					&& Ext.ieVersion >= 8) {
...
Ext.define("Ext.layout.container.Anchor", {
	...
	onLayout : function() {
		this.callParent(arguments);
		var r = this, l = r.getLayoutTargetSize(), a = r.owner, t = r
				.getTarget(), s = l.width, j = l.height, m = t
				.getStyle("overflow"), k = r.getVisibleItems(a), p = k.length, e = [], g, n, h, f, c, d, o, b, q;
		if (s < 20 && j < 20) {
			return
		}
		if (!r.clearEl) {
	...
	configureItem : function(f) {
		...
		if (!f.anchor && f.items && !Ext.isNumber(f.width)
...
Ext.define("Ext.layout.container.CheckboxGroup", {
	...
	getColCount : function() {
		...
		return Ext.isArray(b) ? b.length : (Ext.isNumber(b)
				? b
				: a.items.length)
	}
});
...
Ext.define("Ext.layout.component.Button", {
	...
	onLayout : function(c, n) {
		var k = this, g = Ext.isNumber, d = k.owner, m = d.el, h = d.btnEl, e = d.btnInnerEl, f = d.btnIconEl, i = (d.icon || d.iconCls)
				&& (d.iconAlign == "top" || d.iconAlign == "bottom"), b = d.minWidth, l = d.maxWidth, a, o, j;
		k.getTargetInfo();
		k.callParent(arguments);
		e.unclip();
		k.setTargetSize(c, n);
		if (!g(c)) {
			if (d.text && (Ext.isIE6 || Ext.isIE7) && Ext.isStrict && h
					&& h.getWidth() > 20) {
				o = k.btnFrameWidth;
...
Ext.define("Ext.chart.Label", {
	...
	renderLabels : function() {
		var n = this, K = n.chart, w = K.gradients, q = n.items, G = K.animate, z = n.label, t = z.display, u = z.color, d = []
				.concat(z.field), p = n.labelsGroup, h = (p || 0) && p.length, b = n.chart.store, o = b
				.getCount(), g = (q || 0) && q.length, C = g / o, y = (w || 0)
				&& w.length, l = Ext.draw.Color, J = [], m, I, D, c, x, H, F, e, f, s, v, E, L, r, M, B, A, a;
		if (t == "none") {
			return
		}
		if (g == 0) {
				...
				for (H = 0; H < C; H++) {
					...
					if (v && d[H]) {
						...
						if (z.contrast && v.sprite) {
							...
							if (a && !M) {
								...
								for (F = 0; F < y; F++) {
									...
									if (m.id == a) {
										...
										for (f in m.stops) {
											s++;
											e += l.fromString(m.stops[f].color)
													.getGrayscale()
										}
										B = (e / s) / 255;
										break
							...
							A = l.fromString(E.attr.color || E.attr.fill)
									.getHSL();
							A[2] = B > 0.5 ? 0.2 : 0.8;
							E.setAttributes({
										fill : String(l.fromHSL.apply({}, A))
									}, true)
						}
					}
					D++;
...
Ext.define("Ext.draw.Draw", {
	...
	path2curve : function(j) {
		...
		for (b = 0; b < c; b++) {
			...
			if (g[b].length > 7) {
				...
				e = g[b];
				while (e.length) {
					Ext.Array.splice(g, b++, 0, ["C"].concat(Ext.Array.splice(
									e, 0, 6)))
				}
	...
	interpolatePaths : function(q, k) {
		var h = this, d = h.pathToAbsolute(q), l = h.pathToAbsolute(k), m = {
			...
			if (p[r].length > 7) {
				...
				var s = p[r];
				while (s.length) {
					Ext.Array.splice(p, r++, 0, ["C"].concat(Ext.Array.splice(
									s, 0, 6)))
				}
	...
	snapEndsByDateAndStep : function(h, g, e, a) {
		var d = [h.getFullYear(), h.getMonth(), h.getDate(), h.getHours(),
				h.getMinutes(), h.getSeconds(), h.getMilliseconds()], b = 0, f, c;
		if (a) {
			f = h
		} else {
			switch (e[0]) {
				case Ext.Date.MILLI :
					f = new Date(d[0], d[1], d[2], d[3], d[4], d[5], Math
									.floor(d[6] / e[1])
									* e[1]);
					break;
				case Ext.Date.SECOND :
					f = new Date(d[0], d[1], d[2], d[3], d[4], Math.floor(d[5]
									/ e[1])
									* e[1], 0);
					break;
				case Ext.Date.MINUTE :
					f = new Date(d[0], d[1], d[2], d[3], Math
									.floor(d[4] / e[1])
									* e[1], 0, 0);
					break;
				case Ext.Date.HOUR :
					f = new Date(d[0], d[1], d[2], Math.floor(d[3] / e[1])
									* e[1], 0, 0, 0);
					break;
				case Ext.Date.DAY :
					f = new Date(d[0], d[1], Math.floor(d[2] - 1 / e[1]) * e[1]
									+ 1, 0, 0, 0, 0);
					break;
				case Ext.Date.MONTH :
					f = new Date(d[0], Math.floor(d[1] / e[1]) * e[1], 1, 0, 0,
							0, 0);
					break;
				default :
					f = new Date(Math.floor(d[0] / e[1]) * e[1], 0, 1, 0, 0, 0,
							0);
					break
			}
		}
		...
		defaultHandler : {
			...
			computeDelta : function(i, c, a, f, h) {
				...
				if (d) {
					i = d[1];
					e = d[2];
					if (!this.scrollRE.test(h) && !e
							&& this.pixelDefaultsRE.test(h)) {
						e = "px"
					}
				}
				i = +i || 0;
...
Ext.define("Ext.app.PortalDropZone", {
	...
	notifyOver : function(u, t, v) {
		...
		if (a.fireEvent("validatedrop", f) !== false
				&& a.fireEvent("beforedragover", f) !== false) {
	...
	notifyDrop : function(l, h, g) {
		...
		var j = this.lastPos.c, f = this.lastPos.col, k = this.lastPos.p, a = l.panel, b = this
				.createEvent(l, h, g, f, j, k !== false ? k : j.items
								.getCount());
	...
	constructor : function(a) {
		...
		if (!b.easingFn) {
			...
			if (b.easingFn && b.easingFn.length == 5) {
				c = b.easingFn;
				b.easingFn = Ext.fx.CubicBezier.cubicBezier(+c[1], +c[2],
						+c[3], +c[4])
			}
		}
...
Ext.define("Ext.menu.Manager", {
	...
	onMouseDown : function(f) {
		...
		if (Ext.Date.getElapsed(a) > 50 && d.length > 0
				&& !f.getTarget("." + Ext.baseCSSPrefix + "menu")) {
			b.hideAll();
...
Ext.define("Ext.data.AbstractStore", {
	...
	constructor : function(a) {
		var c = this, b;
		c.addEvents("add", "remove", "update", "datachanged", "beforeload",
				"load", "write", "beforesync", "clear");
		Ext.apply(c, a);
		c.removed = [];
		...
		if (!c.model && c.fields) {
			c.model = Ext.define("Ext.data.Store.ImplicitModel-"
							+ (c.storeId || Ext.id()), {
...
Ext.define("Ext.AbstractComponent", {
	...
	uses : ["Ext.PluginManager", "Ext.ComponentManager", "Ext.Element",
			"Ext.DomHelper", "Ext.XTemplate", "Ext.ComponentQuery",
			"Ext.ComponentLoader", "Ext.EventManager", "Ext.layout.Layout",
			"Ext.layout.component.Auto", "Ext.LoadMask", "Ext.ZIndexManager"],
	statics : {
		AUTO_ID : 1000
	},
	isComponent : true,
	...
	getState : function() {
		var f = this, e = f.ownerCt ? (f.shadowOwnerCt || f.ownerCt)
				.getLayout() : null, g = {
	...
	initFrame : function() {
		...
		var e = this, d = e.getFrameInfo(), b = d.width, a = e
				.getFrameTpl(d.table), c;
		if (e.frame) {
	...
	addUIClsToElement : function(k, a) {
		...
		if (!a && g.frame && !Ext.supports.CSS3BorderRadius) {
			...
			for (f = 0; f < e.length; f++) {
				...
				c = [g.baseCls + "-" + g.ui + "-" + e[f],
						g.baseCls + "-" + g.ui + "-" + k + "-" + e[f]];
				if (b && b.dom) {
					b.addCls(c)
				} else {
		...
		if (!(b[c] instanceof Ext.XTemplate)) {
			...
			do {
				if (d.hasOwnProperty(c)) {
					a = d[c];
					if (a && !(a instanceof Ext.XTemplate)) {
						d[c] = Ext.ClassManager.dynInstantiate("Ext.XTemplate",
								a);
						break
					}
				}
	...
	removeManagedListenerItem : function(b, a, h, d, f, e) {
		...
		if (c) {
			...
			if (c && c.un) {
				if (b
						|| (a.item === h && a.ename === d && (!f || a.fn === f) && (!e || a.scope === e))) {
...
Ext.define("Ext.Component", {
	...
	initDraggable : function() {
		var b = this, a = Ext.applyIf({
					el : b.getDragEl(),
					constrainTo : b.constrain
							? (b.constrainTo || (b.floatParent ? b.floatParent
									.getTargetEl() : b.el.getScopeParent()))
							: undefined
				}, b.draggable);
		if (b.constrain || b.constrainDelegate) {
	...
	getId : function() {
		...
		if (!a.id) {
			b = a.getXType();
			b = b
					? b.replace(Ext.Component.INVALID_ID_CHARS_Re, "-")
					: "ext-comp";
			a.id = b + "-" + a.getAutoId()
		}
		return a.id
		...
		if (c.rendered && c.isVisible()) {
			...
			if (c.fireEvent("beforeshow", c) !== false) {
				...
				if (c.rendered) {
					...
					if (c.ownerCt
							&& !c.floating
							&& !(c.ownerCt.suspendLayout || c.ownerCt.layout.layoutBusy)) {
						c.ownerCt.doLayout()
					}
	...
	hide : function() {
		...
		if (!(a.rendered && !a.isVisible())
				&& a.fireEvent("beforehide", a) !== false) {
			a.hidden = true;
			if (a.rendered) {
				a.onHide.apply(a, arguments);
				if (a.ownerCt
						&& !a.floating
						&& !(a.ownerCt.suspendLayout || a.ownerCt.layout.layoutBusy)) {
					a.ownerCt.doLayout()
				}
			}
		}
...
Ext.define("Ext.resizer.Splitter", {
	...
	renderTpl : ['<tpl if="collapsible===true">',
			'<div id="{id}-collapseEl" class="', Ext.baseCSSPrefix,
			"collapse-el ", Ext.baseCSSPrefix,
	...
	getCollapseTarget : function() {
		var a = this;
		return a.collapseTarget.isComponent
				? a.collapseTarget
				: a.collapseTarget == "prev" ? a.previousSibling() : a
						.nextSibling()
	},
...
Ext.define("Ext.button.Button", {
	...
	getHref : function() {
		...
		return a.href
				? Ext.urlAppend(a.href, Ext.Object.toQueryString(b))
				: false
	},
...
Ext.define("Ext.container.AbstractContainer", {
	...
	initComponent : function() {
		var a = this;
		a
				.addEvents("afterlayout", "beforeadd", "beforeremove", "add",
						"remove");
...
Ext.define("Ext.layout.container.Border", {
	...
	getVisibleItems : function() {
		return Ext.ComponentQuery.query(":not([slideOutAnim])", this
		...
		if (b.north || b.south) {
			...
			if (j.embeddedContainer) {
				...
				Ext.each([j.splitters.north, j.splitters.south], function(i) {
							if (i) {
								i.on("beforedragstart", j.fixHeightConstraints,
										j)
							}
						});
				if (a) {
	...
	onBeforeRegionCollapse : function(g, j, d) {
		...
		if (g.animCollapse && i.initialCollapsedComplete) {
			...
			n.slideOut(i.slideDirection[g.region], {
						duration : Ext.Number.from(g.animCollapse,
	...
	onPlaceHolderToolClick : function(m, s, a, c) {
		...
		if (c.shouldFireBeforeexpand !== false
				&& o.fireEvent("beforeexpand", o, true) === false) {
			return false
		}
...
Ext.define("Ext.layout.container.Box", {
	...
	calculateChildBoxes : function(p, b) {
		var x = this, J = Math, l = J.max, o = Infinity, v, s = x.parallelPrefix, n = x.parallelPrefixCap, M = x.perpendicularPrefix, t = x.perpendicularPrefixCap, C = "min"
				+ n, F = "min" + t, ak = "max" + t, c = b[s] - x.scrollOffset, Z = b[M], ae = x.padding, r = ae[x.parallelBefore], u = r
				+ ae[x.parallelAfter], P = ae[x.perpendicularLeftTop], K = P
				+ ae[x.perpendicularRightBottom], ah = l(0, Z - K), aj = x.innerCt
				.getBorderWidth(x.perpendicularLT + x.perpendicularRB), ab = x.pack == "start", an = x.pack == "center", E = x.pack == "end", ad = Ext.Number.constrain, N = p.length, d = 0, ai = 0, al = 0, w = 0, G = 0, Q = [], L = [], ag, af, h, aa, A, B, am, Y, W, X, m, e, z, I, y, O, j, R, ac, f, D, T, a, q, k, H, S, V, g, U;
		for (af = 0; af < N; af++) {
			h = p[af];
			A = h[M];
			...
			G = l(G, l(A, h[F] || 0) + B[x.perpendicularLeftTop]
							+ B[x.perpendicularRightBottom]);
			W[s] = aa || v;
			W.dirtySize = h.componentLayout.lastComponentSize
					? (W[s] !== h.componentLayout.lastComponentSize[s])
					: false;
			W[M] = A || v;
			Q.push(W)
		}
...
Ext.define("Ext.layout.container.Accordion", {
	...
	itemCls : Ext.baseCSSPrefix + "box-item " + Ext.baseCSSPrefix
			+ "accordion-item",
	align : "stretch",
	fill : true,
	autoWidth : true,
		...
		for (; d < f; d++) {
			...
			if (!b.rendered) {
				...
				if (g.fill) {
					...
					g.owner.mon(b, {
								show : g.onComponentShow,
								beforeexpand : g.onComponentExpand,
								beforecollapse : g.onComponentCollapse,
								scope : g
							})
				} else {
					delete b.flex;
...
Ext.define("Ext.panel.Header", {
	...
	renderTpl : [
			...
			'</tpl>"', '<tpl if="bodyStyle"> style="{bodyStyle}"</tpl>></div>'],
	initComponent : function() {
	...
	addUIClsToElement : function(b, f) {
		var e = this, a = e.callParent(arguments), d = [
				e.baseCls + "-body-" + b, e.baseCls + "-body-" + e.ui + "-" + b], g, c;
		if (!f && e.rendered) {
			if (e.bodyCls) {
...
Ext.define("Ext.panel.AbstractPanel", {
	...
	addUIClsToElement : function(b, f) {
		var e = this, a = e.callParent(arguments), d = [Ext.baseCSSPrefix + b,
				e.baseCls + "-body-" + b, e.baseCls + "-body-" + e.ui + "-" + b], g, c;
		if (!f && e.rendered) {
			if (e.bodyCls) {
				e.body.addCls(e.bodyCls)
...
Ext.define("Ext.panel.Panel", {
	...
	initComponent : function() {
		var b = this, a;
		b.addEvents("beforeclose", "beforeexpand", "beforecollapse", "expand",
				"collapse", "titlechange", "iconchange");
		...
		if (a.fbar) {
			...
			if (e) {
				...
				c.defaults = function(g) {
					var h = b || {};
					if ((!g.xtype || g.xtype === "button" || (g.isComponent && g
							.isXType("button")))
							&& !("minWidth" in h)) {
						h = Ext.apply({
									minWidth : e
	...
	collapse : function(t, f, h) {
		var u = this, s = Ext.Component, j = u.getHeight(), k = u.getWidth(), v, a = 0, p = u.dockedItems.items, q = p.length, o = 0, r, g, n = {
			from : {
				height : j,
				width : k
			},
		...
		switch (t) {
			...
			case s.DIRECTION_BOTTOM :
				...
				for (; o < q; o++) {
					r = p[o];
					if (r.isVisible()) {
						if (r.isXType("header", true)
								&& (!r.dock || r.dock == "top" || r.dock == "bottom")) {
							e = r
						} else {
				...
				if (t == Ext.Component.DIRECTION_BOTTOM) {
					g = u.getPosition()[1]
							- Ext.fly(u.el.dom.offsetParent).getRegion().top;
					n.from.top = g
				}
				break;
			...
			case s.DIRECTION_RIGHT :
				...
				if (t == Ext.Component.DIRECTION_RIGHT) {
					g = u.getPosition()[0]
							- Ext.fly(u.el.dom.offsetParent).getRegion().left;
					n.from.left = g
				}
				break;
			default :
		...
		if (e && e.rendered) {
			...
			if (u.border
					&& (!u.frame || (u.frame && Ext.supports.CSS3BorderRadius))) {
			...
			e = {
				...
				cls : u.baseCls + "-collapsed-placeholder  "
						+ Ext.baseCSSPrefix + "docked " + u.baseCls + "-"
						+ u.ui + "-collapsed",
				renderTo : u.el
			};
		...
		if (u.border
				&& (!u.frame || (u.frame && Ext.supports.CSS3BorderRadius))) {
			u.reExpander.addClsWithUI(u.collapsedCls + "-border-"
					+ u.reExpander.dock)
		}
		if (t == Ext.Component.DIRECTION_RIGHT) {
			n.to.left = g + (k - a)
		...
		u.collapseMemento.capture(["width", "height", "minWidth", "minHeight",
				"layoutManagedHeight", "layoutManagedWidth"]);
		u.savedFlex = u.flex;
		u.minWidth = 0;
		u.minHeight = 0;
		delete u.flex;
	...
	expand : function(b) {
		...
		var e = 0, c = f.hiddenDocked.length, h = f.expandDirection, j = f
				.getHeight(), a = f.getWidth(), g, d;
		if (b && f.collapseTool) {
		...
		if ((h == Ext.Component.DIRECTION_TOP)
				|| (h == Ext.Component.DIRECTION_BOTTOM)) {
			...
			if ((h == Ext.Component.DIRECTION_LEFT)
					|| (h == Ext.Component.DIRECTION_RIGHT)) {
...
Ext.define("Ext.window.Window", {
	...
	syncMonitorWindowResize : function() {
		var b = this, c = b._monitoringResize, d = b.monitorResize
				|| b.constrain || b.constrainHeader || b.maximized, a = b.hidden
				|| b.destroying || b.isDestroyed;
		if (d && !a) {
			if (!c) {
...
Ext.define("Ext.app.PortalPanel", {
			...
			beforeLayout : function() {
				...
				for (; c < a; c++) {
					d = b[c];
					d.columnWidth = 1 / a;
					d.removeCls(["x-portal-column-first",
							"x-portal-column-last"])
				}
			...
			initEvents : function() {
				this.callParent();
				this.dd = Ext.create("Ext.app.PortalDropZone", this,
						this.dropConfig)
			},
			beforeDestroy : function() {
				if (this.dd) {
...
Ext.define("Ext.panel.Table", {
	...
	initComponent : function() {
		...
		if (!g.hasView) {
			...
			if (d) {
				g.verticalScroller = Ext.ComponentManager.create(g
			...
			if (c) {
				g.horizontalScroller = Ext.ComponentManager.create(g
			...
			this.relayEvents(b, ["beforeitemmousedown", "beforeitemmouseup",
							...
							"beforeitemcontextmenu", "itemmousedown",
							"itemmouseup", "itemmouseenter", "itemmouseleave",
							"itemclick", "itemdblclick", "itemcontextmenu",
							"beforecontainermousedown",
							"beforecontainermouseup",
							"beforecontainermouseover",
							"beforecontainermouseout", "beforecontainerclick",
							"beforecontainerdblclick",
							"beforecontainercontextmenu", "containermouseup",
							"containermouseover", "containermouseout",
							"containerclick", "containerdblclick",
							"containercontextmenu", "selectionchange",
							"beforeselect", "select", "beforedeselect",
							"deselect"])
		}
		g.callParent(arguments)
	},
	...
	applyState : function(d) {
		...
		if (e) {
			if (a.remoteSort) {
				a.sorters.add(Ext.create("Ext.util.Sorter", {
							property : e.property,
	...
	getView : function() {
		...
		if (!a.view) {
			...
			a.view = a.createComponent(Ext.apply({}, a.viewConfig, {
						deferInitialRefresh : a.deferRowRender,
						xtype : a.viewType,
						store : a.store,
						headerCt : a.headerCt,
						selModel : b,
	...
	determineScrollbars : function() {
		...
		var h = this, i = h.view, d, a, e, j, l, f, g = h.verticalScroller, c = h.horizontalScroller, k = (g
				&& g.ownerCt === h ? 1 : 0)
				| (c && c.ownerCt === h ? 2 : 0), b = 0;
		if (!h.collapsed && i && i.viewReady) {
			d = i.el.getSize();
			j = d.width + ((k & 1) ? g.width : 0);
			f = d.height + ((k & 2) ? c.height : 0);
			e = (h.headerCt.query("[flex]").length && !h.headerCt.layout.tooNarrow)
					? 0
					: h.headerCt.getFullWidth();
			if (g && g.el) {
	...
	showHorizontalScroller : function() {
		var a = this;
		if (a.verticalScroller) {
			a.verticalScroller.setReservedSpace(Ext.getScrollbarSize().height
					- 1)
		}
		if (a.horizontalScroller && a.horizontalScroller.ownerCt !== a) {
	...
	onMouseWheel : function(m) {
		var n = this, j = n.getVerticalScroller(), o = n
				.getHorizontalScroller(), b = -n.scrollDelta, c = m
				.getWheelDeltas(), h = b * c.x, g = b * c.y, l, p, a, q, d, i, k, f;
		if (o) {
			p = o.scrollEl;
			if (p) {
				q = p.dom;
	...
	getLhsMarker : function() {
		var a = this;
		if (!a.lhsMarker) {
			a.lhsMarker = Ext.DomHelper.append(a.el, {
	...
	getRhsMarker : function() {
		var a = this;
		if (!a.rhsMarker) {
			a.rhsMarker = Ext.DomHelper.append(a.el, {
						cls : Ext.baseCSSPrefix + "grid-resize-marker"
					}, true)
		}
		return a.rhsMarker
	...
	getSelectionModel : function() {
		...
		if (!this.selModel.hasRelaySetup) {
			this.relayEvents(this.selModel, ["selectionchange", "beforeselect",
							"beforedeselect", "select", "deselect"]);
	...
	onHorizontalScroll : function(d, e) {
		var b = this.getScrollerOwner(), c = b.query("tableview"), a = c[1]
				|| c[0];
	...
	getTargetXY : function() {
		...
		if (i.anchor) {
			...
			var c = i.getOffsets(), m = (i.anchorToTarget && !i.trackMouse)
					...
					: i.targetXY, a = Ext.Element.getViewWidth() - 5, h = Ext.Element
					.getViewHeight()
...
Ext.define("Ext.draw.Surface", {
	...
	insertByZIndex : function(i) {
		var f = this, d = f.items.items, c = d.length, j = Math.ceil, g = i.attr.zIndex, h = c, b = h
				- 1, e = 0, a;
	...
	getPathellipse : function(c) {
		var b = c.attr;
		return this.ellipsePath(b.x, b.y, b.radiusX || (b.width / 2) || 0,
				b.radiusY || (b.height / 2) || 0)
	},
...
Ext.define("Ext.chart.axis.Axis", {
	...
	getRange : function() {
		var r = this, d = r.chart.getChartStore(), j = r.fields, f = j.length, c = Math, t = c.max, q = c.min, h = false, m = isNaN(r.minimum)
				? Infinity
				: r.minimum, o = isNaN(r.maximum) ? -Infinity : r.maximum, s = 0, p, n, k, a, e, b = [], g = r.chart.series.items;
	...
	calcEnds : function() {
		...
		d = Ext.draw.Draw.snapEnds(g, a, h.majorTickSteps !== false
						? (h.majorTickSteps + 1)
						: h.steps);
		e = d.from;
		f = d.to;
	...
	drawHorizontalLabels : function() {
		...
		f = Math.floor(Math.abs(Math.sin(c.rotate
...
Ext.define("Ext.chart.series.Line", {
	...
	drawSeries : function() {
		var ak = this, au = ak.chart, S = au.axes, ao = au.getChartStore(), V = ao
				.getCount(), u = ak.chart.surface, am = {}, R = ak.group, K = ak.showMarkers, aA = ak.markerGroup, D = au.shadow, C = ak.shadowGroups, X = ak.shadowAttributes, O = ak.smooth, q = C.length, ar = ["M"], T = ["M"], d = ["M"], b = ["M"], J = au.markerIndex, ai = []
				.concat(ak.axis), ah, av = [], ag = {}, aa = [], v = {}, I = false, Q = [], az = ak.markerStyle, Z = ak.style, t = ak.colorArrayStyle, P = t
				&& t.length || 0, L = Ext.isNumber, aw = ak.seriesIdx, g = ak
				.getAxesForXAndYFields(), l = g.xAxis, ay = g.yAxis, ac, h, ab, ad, A, c, ae, H, G, f, e, s, r, W, N, M, at, m, F, E, aB, n, p, B, a, Y, af, z, aq, w, ap, o, ax, an, al, U, k, aj;
		...
		if (ak.xField && !L(z) && (l == "bottom" || l == "top") && !S.get(l)) {
			m = Ext.create("Ext.chart.axis.Axis", {
						chart : au,
						fields : [].concat(ak.xField)
					}).calcEnds();
			z = m.from;
			aq = m.to
		}
		...
		ak.eachRecord(function(j, x) {
					...
					if (typeof p == "string" || typeof p == "object"
							&& !Ext.isDate(p) || l && S.get(l)
							&& S.get(l).type == "Category") {
						if (p in ag) {
							p = ag[p]
						} else {
							p = ag[p] = x
					...
					if (typeof B == "string" || typeof B == "object"
							&& !Ext.isDate(B) || ay && S.get(ay)
							&& S.get(ay).type == "Category") {
						B = x
					}
					Q.push(x);
					av.push(p);
		...
		for (N = 0; N < at; N++) {
			...
			if (au.animate && au.resizing && ak.line) {
				ak.line.setAttributes({
							path : ar
						}, true);
				if (ak.fillPath) {
					ak.fillPath.setAttributes({
								path : ar,
								opacity : 0.2
							}, true)
				}
			...
			if (K) {
				E = aA.getAt(k++);
				if (!E) {
					E = Ext.chart.Shape[U](u, Ext.apply({
										group : [R, aA],
										x : 0,
										y : 0,
										translate : {
											x : +(f || H),
											y : e || (am.y + am.height / 2)
										},
										value : '"' + p + ", " + B + '"',
										zIndex : 4000
									}, an));
		...
		if (!ak.line) {
			ak.line = u.add(Ext.apply({
						type : "path",
						group : R,
						path : ar,
						stroke : al.stroke || al.fill
					}, al || {}));
			if (D) {
		...
		if (ak.fill) {
			c = d.concat([["L", H, am.y + am.height],
					["L", s, am.y + am.height], ["L", s, r]]);
			if (!ak.fillPath) {
				ak.fillPath = u.add({
							group : R,
							type : "path",
							opacity : al.opacity || 0.3,
							fill : al.fill || t[aw % P],
							path : ar
						})
			}
		...
		if (au.animate) {
			...
			o.show(true);
			if (au.markerIndex && ak.previousPath) {
				ak.animation = ax = ak.onAnimate(o, {
							to : ae,
							from : {
								path : ad
							}
	...
	highlightItem : function() {
		...
		if (a.line && !a.highlighted) {
			...
			a.line.__anim = Ext.create("Ext.fx.Anim", {
						target : a.line,
						to : {
							"stroke-width" : a.line.__strokeWidth + 3
						}
					});
			a.highlighted = true
		}
	...
	unHighlightItem : function() {
		...
		if (a.line && a.highlighted) {
			a.line.__anim = Ext.create("Ext.fx.Anim", {
						target : a.line,
						to : {
							"stroke-width" : a.line.__strokeWidth
						}
					});
			a.highlighted = false
		}
...
Ext.define("Ext.selection.Model", {
	...
	selectAll : function(b) {
		var e = this, d = e.store.getRange(), c = 0, a = d.length, f = e
				.getSelection().length;
		e.bulkChange = true;
		for (; c < a; c++) {
	...
	deselectAll : function(b) {
		var e = this, d = e.getSelection(), c = 0, a = d.length, f = e
				.getSelection().length;
		e.bulkChange = true;
		for (; c < a; c++) {
			e.doDeselect(d[c], b)
				...
				if (d.ctrlKey && b.isSelected(a)) {
					...
					if (d.shiftKey && b.lastFocused) {
						...
						if (d.ctrlKey) {
							...
							if (b.isSelected(a) && !d.shiftKey && !d.ctrlKey
...
Ext.define("Ext.selection.DataViewModel", {
			...
			enableKeyNav : true,
			constructor : function(a) {
				this.addEvents("beforedeselect", "beforeselect", "deselect",
						"select");
				this.callParent(arguments)
			},
			bindComponent : function(a) {
			...
			onSelectChange : function(b, e, d, g) {
				...
				if ((d || f.fireEvent("before" + c, f, b)) !== false
						&& g() !== false) {
					if (e) {
						a.onItemSelect(b)
					} else {
...
Ext.define("Ext.view.AbstractView", {
			...
			initComponent : function() {
				...
				c.addEvents("beforerefresh", "refresh", "viewready",
						"itemupdate", "itemadd", "itemremove");
				c.addCmpEvents();
			...
			getSelectionModel : function() {
				...
				if (!a.selModel.hasRelaySetup) {
					a.relayEvents(a.selModel, ["selectionchange",
									"beforeselect", "beforedeselect", "select",
									"deselect"]);
			...
			collectData : function(c, f) {
				...
				for (; d < a; d++) {
					b = c[d];
					e[e.length] = this.prepareData(b[b.persistenceProperty], f
									+ d, b)
				}
				return e
			},
...
Ext.define("Ext.view.View", {
	...
	processUIEvent : function(g) {
		var h = this, j = g.getTarget(h.getItemSelector(), h.getTargetEl()), a = this
				.statics().EventMap, f, c, i = g.type, d = h.mouseOverItem, b;
		if (!j) {
		...
		if (j) {
			...
			if ((h["onBeforeItem" + a[b]](c, j, f, g) === false)
					|| (h.fireEvent("beforeitem" + b, h, c, j, f, g) === false)
					|| (h["onItem" + a[b]](c, j, f, g) === false)) {
				return false
			}
			h.fireEvent("item" + b, h, c, j, f, g)
		} else {
			if ((h.processContainerEvent(g) === false)
					|| (h["onBeforeContainer" + a[i]](g) === false)
					|| (h.fireEvent("beforecontainer" + i, h, g) === false)
					|| (h["onContainer" + a[i]](g) === false)) {
				return false
			}
...
Ext.define("Ext.view.Table", {
		...
		if (a.invalidateScrollerOnRefresh) {
			if (Ext.isGecko) {
				if (!a.scrollToTopTask) {
					a.scrollToTopTask = Ext.create("Ext.util.DelayedTask",
							a.scrollToTop, a)
				}
	...
	onHeaderResize : function(e, a, d) {
		...
		if (b) {
			...
			b.select("." + Ext.baseCSSPrefix + "grid-col-resizer-" + e.id)
					.setWidth(a);
			b.select("." + Ext.baseCSSPrefix + "grid-table-resizer")
					.setWidth(c.headerCt.getFullWidth());
	...
	focusCell : function(g) {
		var i = this, j = i.getCellByPosition(g), b = i.el, d = 0, e = 0, c = b
				.getRegion(), a = i.ownerCt, h, f;
		if (j) {
			h = j.getRegion();
			if (h.top < c.top) {
				d = h.top - c.top
	...
	processItemEvent : function(b, l, i, d) {
		var g = this, j = d.getTarget(g.cellSelector, l), h = j
				? j.cellIndex
				: -1, a = g.statics().EventMap, c = g.getSelectionModel(), f = d.type, k;
		...
		return !((g["onBeforeCell" + a[f]](j, h, b, l, i, d) === false)
				|| (g.fireEvent("beforecell" + f, g, j, h, b, l, i, d) === false)
				|| (g["onCell" + a[f]](j, h, b, l, i, d) === false) || (g
				.fireEvent("cell" + f, g, j, h, b, l, i, d) === false))
	},
	...
	getPositionByEvent : function(f) {
		var d = this, b = f.getTarget(d.cellSelector), c = f
				.getTarget(d.itemSelector), a = d.getRecord(c), g = d
				.getHeaderByCell(b);
	...
	walkCells : function(k, l, g, m, a, n) {
		var i = this, o = k.row, d = k.column, j = i.store.getCount(), f = i
				.getFirstVisibleColumnIndex(), b = i
...
Ext.define("Ext.data.Model", {
	...
	requires : ["Ext.ModelManager", "Ext.data.IdGenerator", "Ext.data.Field",
			"Ext.data.Errors", "Ext.data.Operation", "Ext.data.validations",
	...
	onClassExtended : function(a, b) {
		...
		b.onBeforeClassCreated = function(d, x) {
			var w = this, y = Ext.getClassName(d), l = d.prototype, q = d.prototype.superclass, e = x.validations
					|| [], n = x.fields || [], s = x.associations || [], r = x.belongsTo, m = x.hasMany, o = x.idgen, u = new Ext.util.MixedCollection(
					false, function(i) {
						return i.name
	...
	copy : function(a) {
		var b = this;
		return new b.self(Ext.apply({}, b[b.persistenceProperty]), a
						|| b.internalId)
	},
	setProxy : function(a) {
		if (!a.isProxy) {
...
Ext.define("Ext.data.Store", {
	...
	alias : "store.store",
	requires : ["Ext.data.StoreManager", "Ext.ModelManager", "Ext.data.Model",
			"Ext.util.Grouper"],
	...
	constructor : function(b) {
		...
		var d = this, f = b.groupers || d.groupers, a = b.groupField
				|| d.groupField, c, e;
		...
		if (e) {
			...
			if (d.autoLoad) {
				Ext.defer(d.load, 10, d, [typeof d.autoLoad === "object"
								? d.autoLoad
								: undefined])
			}
		}
...
Ext.define("Ext.Layer", {
	...
	constructor : function(b, a) {
		b = b || {};
		var c = this, d = Ext.DomHelper, f = b.parentEl, e = f
				? Ext.getDom(f)
				: document.body, g = b.hideMode;
		if (a) {
			c.dom = Ext.getDom(a)
		}
		if (!c.dom) {
	...
	constrainXY : function() {
		if (this.constrain) {
			var f = Ext.Element.getViewWidth(), b = Ext.Element.getViewHeight(), k = Ext
					.getDoc().getScroll(), j = this.getXY(), g = j[0], e = j[1], a = this.shadowOffset, i = this.dom.offsetWidth
...
Ext.define("Ext.resizer.SplitterTracker", {
	...
	onBeforeStart : function(g) {
		var d = this, f = d.getPrevCmp(), a = d.getNextCmp(), c = d
				.getSplitter().collapseEl, b;
		...
		b.unselectable();
		b.setSize(Ext.Element.getViewWidth(true), Ext.Element
						.getViewHeight(true));
		b.show();
	...
	calculateConstrainRegion : function() {
		var f = this, a = f.getSplitter(), g = a.getWidth(), h = a.defaultSplitMin, b = a.orientation, d = f.prevBox, i = f
				.getPrevCmp(), c = f.nextBox, e = f.getNextCmp(), k, j;
		if (b === "vertical") {
			k = Ext.create("Ext.util.Region", d.y, (i.maxWidth ? d.x
							+ i.maxWidth : c.right - (e.minWidth || h))
							+ g, d.bottom, d.x + (i.minWidth || h));
			j = Ext.create("Ext.util.Region", c.y, c.right - (e.minWidth || h),
					c.bottom, (e.maxWidth ? c.right - e.maxWidth : d.x
							+ (d.minWidth || h))
							- g)
		} else {
			k = Ext.create("Ext.util.Region", d.y + (i.minHeight || h),
					d.right, (i.maxHeight ? d.y + i.maxHeight : c.bottom
							- (e.minHeight || h))
							+ g, d.x);
			j = Ext.create("Ext.util.Region", (e.maxHeight ? c.bottom
							- e.maxHeight : d.y + (i.minHeight || h))
							- g, c.right, c.bottom - (e.minHeight || h), c.x)
		}
		return k.intersect(j)
	},
	performResize : function(g) {
		var i = this, c = i.getOffset("dragTarget"), a = i.getSplitter(), d = a.orientation, j = i
				.getPrevCmp(), h = i.getNextCmp(), b = a.ownerCt, f = b
				.getLayout();
		b.suspendLayout = true;
		if (d === "vertical") {
			if (j) {
	...
	onDrag : function(f) {
		var c = this, g = c.getOffset("dragTarget"), d = c.getSplitter(), b = d
				.getEl(), a = d.orientation;
		if (a === "vertical") {
...
Ext.define("Ext.selection.RowModel", {
	...
	onSelectChange : function(f, c, k, a) {
		var h = this, l = h.views, d = l.length, j = h.store, b = j.indexOf(f), g = c
				? "select"
				: "deselect", e = 0;
		if ((k || h.fireEvent("before" + g, h, f, b)) !== false
				&& a() !== false) {
			for (; e < d; e++) {
				if (c) {
					l[e].onRowSelect(b, k)
...
Ext.define("Ext.grid.Scroller", {
	...
	renderTpl : [
			'<div class="' + Ext.baseCSSPrefix
					+ 'scroller-ct" id="{baseId}_ct">',
			'<div class="' + Ext.baseCSSPrefix
					+ 'stretcher" id="{baseId}_stretch"></div>', "</div>"],
	initComponent : function() {
	...
	setScrollTop : function(c) {
		...
		if (a) {
			return a.scrollTop = Ext.Number.constrain(c, 0, a.scrollHeight
							- a.clientHeight)
		}
	},
	setScrollLeft : function(c) {
		var b = this.scrollEl, a = b && b.dom;
		if (a) {
			return a.scrollLeft = Ext.Number.constrain(c, 0, a.scrollWidth
							- a.clientWidth)
		}
	},
...
Ext.define("Ext.grid.header.Container", {
	...
	onHeaderShow : function(g, b) {
		...
		if (d) {
			...
			c = e.length;
			if ((k.getVisibleGridColumns().length > 1) && k.disabledMenuItems
					&& k.disabledMenuItems.length) {
				if (c == 1) {
	...
	getMenuItems : function() {
		var c = this, b = [], a = c.enableColumnHide
				? c.getColumnMenu(c)
				: null;
		if (c.sortable) {
			b = [{
	...
	prepareData : function(h, c, j, l, a) {
		...
		for (; f < e; f++) {
			...
			g[o + "-modified"] = j.isModified(i.dataIndex) ? Ext.baseCSSPrefix
					+ "grid-dirty-cell" : "";
			g[o + "-tdCls"] = b.tdCls;
			g[o + "-tdAttr"] = b.tdAttr;
			g[o + "-style"] = b.style;
...
Ext.define("Ext.tip.QuickTip", {
			...
			onTargetOver : function(h) {
				...
				if (f.activeTarget
						&& ((g == f.activeTarget.el) || Ext
								.fly(f.activeTarget.el).contains(g))) {
					f.clearTimer("hide");
					f.show();
					return
				}
				...
				if (b) {
					...
					f.activeTarget = {
						el : g,
						text : b.text,
						width : +i.getAttribute(d + c.width) || null,
						autoHide : a != "user" && a !== "false",
						title : i.getAttribute(d + c.title),
						cls : i.getAttribute(d + c.cls),
						align : i.getAttribute(d + c.align)
					};
...
Ext.define("Ext.menu.Item", {
	...
	onRender : function(a, d) {
		...
		Ext.applyIf(b.renderData, {
					...
					icon : b.icon || c,
					iconCls : b.iconCls
							+ (b.checkChangeDisabled ? " " + b.disabledCls : ""),
...
Ext.define("Ext.draw.Matrix", {
	...
	toFilter : function() {
		var a = this;
		return "progid:DXImageTransform.Microsoft.Matrix(sizingMethod='auto expand',FilterType=bilinear,M11="
				+ a.get(0, 0)
				+ ", M12="
				+ a.get(0, 1)
				+ ", M21="
...
Ext.define("Ext.grid.Lockable", {
	...
	processColumns : function(f) {
		...
		for (; e < a; ++e) {
			...
			if (!g.headerId) {
				g.headerId = (g.initialConfig || g).id
						|| ("L" + (++this.headerCounter))
			}
		}
		return {
			lockedWidth : b,
	...
	getSpacerEl : function() {
		...
		if (!d.spacerEl) {
			...
			c = a.el;
			d.spacerEl = Ext.DomHelper.append(c, {
						cls : d.spacerHidden
								? (Ext.baseCSSPrefix + "hidden")
								: "",
						style : "height: " + b + "px;"
					}, true)
		}
	...
	getMenuItems : function(b) {
		var f = this, g = f.unlockText, h = f.lockText, c = Ext.baseCSSPrefix
				+ "hmenu-unlock", e = Ext.baseCSSPrefix + "hmenu-lock", a = Ext.Function
	...
	getColumnsState : function() {
		var b = this, a = b.lockedGrid.headerCt.getColumnsState(), c = b.normalGrid.headerCt
				.getColumnsState();
		return a.concat(c)
	},
...
Ext.define("Ext.menu.Menu", {
	...
	minWidth : 120,
	initComponent : function() {
		var b = this, d = Ext.baseCSSPrefix, a = [d + "menu"], c = b.bodyCls
				? [b.bodyCls]
				: [];
	...
	doConstrain : function() {
		...
		if (e > h && h > 0) {
			...
			if (f.showSeparator) {
				f.iconSepEl
						.setHeight(f.layout.getRenderTarget().dom.scrollHeight)
			}
		}
...
Ext.define("Ext.draw.engine.Svg", {
	...
	applyAttrs : function(q) {
		var l = this, c = q.el, p = q.group, h = q.attr, r = l.parsers, f = l.gradientsMap
				|| {}, j = Ext.isSafari && !Ext.isStrict, e, g, k, o, d, n, b, a, m;
		if (p) {
			e = [].concat(p);
			k = e.length;
			for (g = 0; g < k; g++) {
		...
		if (q.type == "circle" || q.type == "ellipse") {
			...
			if (q.type == "rect") {
				...
				if (q.type == "path" && o.d) {
					o.d = Ext.draw.Draw.pathToString(Ext.draw.Draw
							.pathToAbsolute(o.d))
				}
			}
		}
	...
	addGradient : function(g) {
		...
		var e = this, d = g.stops.length, a = g.vector, k = Ext.isSafari
				&& !Ext.isStrict, h, f, j, c, b;
		b = e.gradientsMap || {};
		if (!k) {
			if (g.type == "linear") {
...
Ext.define("Ext.draw.engine.Vml", {
	...
	createSpriteElement : function(g) {
		var e = this, d = g.attr, f = g.type, i = e.zoom, b = g.vml
				|| (g.vml = {}), j = Math.round, c = e.createNode("shape"), k, h, a;
		c.coordsize = i + " " + i;
		...
		for (c = j.transformations, a = c.length; f < a; f++) {
			...
			switch (d.type) {
				...
				case "rotate" :
					...
					break;
				case "scale" :
					h.scale(d.x || d.scale, d.y || d.scale, d.centerX,
							d.centerY);
					break
			}
		}
	...
	setSimpleCoords : function(m, k, j, p, o, e) {
		var i = this, l = m.matrix, d = m.el.dom, a = d.style, c = 1, f = "", n = d
				.getElementsByTagName("fill")[0], h = i.zoom / k, g = i.zoom
				/ j, b;
		if (!k || !j) {
			return
		}
	...
	transform : function(l) {
		var i = this, b = l.el, p = l.skew, e = b.dom, f = e.style, k = i
				.extractTransform(l).clone(), j, o = i.zoom, n = e
				.getElementsByTagName("fill")[0], a = !String(l.fill)
				.indexOf("url("), d, h;
		if (l.type != "image" && p && !a) {
			p.matrix = k.toString();
			d = k.offset();
			...
			if (j.isSimple) {
				f.filter = "";
				i.setSimpleCoords(l, j.scaleX, j.scaleY, j.translateX,
						j.translateY, j.rotate / Math.PI * 180)
			} else {
				f.filter = k.toFilter();
...
Ext.define("Ext.resizer.ResizeTracker", {
	...
	updateDimensions : function(r, l) {
		var s = this, c = s.activeResizeHandle.region, f = s
				.getOffset(s.constrainTo ? "dragTarget" : null), j = s.startBox, g, o = 0, t = 0, i, p, a = 0, v = 0, u, m = f[0] < 0
				? "right"
				: "left", q = f[1] < 0 ? "down" : "up", h, b;
		switch (c) {
			case "south" :
				t = f[1];
				b = 2;
			...
			if (a.lockableInjected) {
				...
				if (b > f) {
					...
					if (f > b) {
						d = f;
						this.align = "stretch";
						a.setCalculatedSize(a.getWidth(), g.getHeight(),
								a.ownerCt);
						delete this.align;
						c = a.up("tablepanel")
					}
				...
				j = m[0].layout.getLayoutItems().concat(m[1].layout
						.getLayoutItems())
			} else {
...
Ext.define("Ext.grid.column.Column", {
	...
	renderTpl : '<div id="{id}-titleContainer" class="'
			+ Ext.baseCSSPrefix
			+ 'column-header-inner"><span id="{id}-textEl" class="'
			+ Ext.baseCSSPrefix
			+ 'column-header-text">{text}</span><tpl if="!values.menuDisabled"><div id="{id}-triggerEl" class="'
	...
	getColumnState : function() {
		...
		b
				.savePropsToState(["hidden", "sortable", "locked", "flex",
								"width"], c);
		if (b.isGroupHeader) {
			b.items.each(function(d) {
	...
	onRender : function() {
		...
		if (a
				&& (!b.sortable || a.sortableColumns === false)
				&& !b.groupable
				&& !b.lockable
				&& (a.enableColumnHide === false || !b.getOwnerHeaderCt()
						.getHideableColumns().length)) {
			b.menuDisabled = true
		}
		b.callParent(arguments)
	},
		...
		if (this.headerCt.dragging) {
			...
			if (f) {
				...
				if (d) {
					...
					if (d
							&& !(d.fixed || (d.resizable === false) || this.disabled)) {
						this.activeHd = d;
...
Ext.define("Ext.ShadowPool", {
	...
	pull : function() {
		...
		if (!a) {
			a = Ext.get(Ext.DomHelper.insertHtml("beforeBegin",
...
Ext.define("Ext.Shadow", {
	...
	show : function(c) {
		...
		if (Ext.isIE && !Ext.supports.CSS3BoxShadow) {
			b.el.dom.style.filter = "progid:DXImageTransform.Microsoft.alpha(opacity="
					+ b.opacity
					+ ") progid:DXImageTransform.Microsoft.Blur(pixelradius="
					+ (b.offset) + ")"
		}
		b.realign(c.getLeft(true), c.getTop(true), c.dom.offsetWidth,
				c.dom.offsetHeight);
