...
public class Message {
    ...
    public static String mapToJson(HashMap<java.lang.String, java.lang.Object> map, List<com.itg.extjstest.domain.Message> messages) {
        ...
        String resultJson = new JSONSerializer().exclude("*.class").include("messages").include("messages.specifications").transform(new DateTransformer("yyyy-MM-dd HH:mm:ss"), Date.class).serialize(map);
        return resultJson;
    }
    ...
    public String validateObject(Locale locale) {
        ...
        if (violations.size() != 0) {
            ResourceBundle bundle = ResourceBundle.getBundle("validateMessages", locale);
            for (ConstraintViolation<Message> violation : violations) {
                buf.append("-" + bundle.getString("Message." + violation.getPropertyPath().toString()));
